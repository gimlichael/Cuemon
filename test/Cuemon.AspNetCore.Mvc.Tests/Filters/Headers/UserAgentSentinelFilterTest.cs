...
namespace Cuemon.AspNetCore.Mvc.Filters.Headers
{
	public class UserAgentSentinelFilterTest : Test
	{
		...
		{
			using (var filter = WebApplicationTestFactory.Create(services => services.AddControllers(o => { o.Filters.AddUserAgentSentinel(); }).AddApplicationPart(typeof(FakeController).Assembly), app =>
				   {
					   app.UseRouting();
					   app.UseEndpoints(routes => { routes.MapControllers(); });
				   }))
