...
namespace Cuemon.Extensions.Runtime.Serialization
{
    ...
    public static class FormatterExtensions
    {
        ...
        public static IConvertible UseConvertibleFormatter(this IHierarchy<DataPair> hierarchy)
        {
            ...
            return ConvertFactory.FromObject().ChangeType(i.Value, ConvertibleTypes.Single(pair => pair.Value == i.Name).Key) as IConvertible;
        }
        ...
        private static IEnumerable<KeyValuePair<object, object>> ParseDictionaryItem(this IEnumerable<IHierarchy<DataPair>> items, Type[] valueTypes)
        {
            ...
            if (valueTypeInfo.IsPrimitive)
            {
                return dicItems.Select(i => new KeyValuePair<object, object>(ConvertFactory.FromObject().ChangeType(i.Key.Instance.Value, valueTypes[0]), i.Value.UseConvertibleFormatter()));
            }

            if (valueType == typeof(Uri))
            {
                return dicItems.Select(i => new KeyValuePair<object, object>(ConvertFactory.FromObject().ChangeType(i.Key.Instance.Value, valueTypes[0]), i.Value.UseUriFormatter()));
            }

            if (valueType == typeof(decimal))
            {
                return dicItems.Select(i => new KeyValuePair<object, object>(ConvertFactory.FromObject().ChangeType(i.Key.Instance.Value, valueTypes[0]), i.Value.UseDecimalFormatter()));
            }

            if (valueType == typeof(string))
            {
                return dicItems.Select(i => new KeyValuePair<object, object>(ConvertFactory.FromObject().ChangeType(i.Key.Instance.Value, valueTypes[0]), i.Value.UseStringFormatter()));
            }

            if (valueType == typeof(Guid))
            {
                return dicItems.Select(i => new KeyValuePair<object, object>(ConvertFactory.FromObject().ChangeType(i.Key.Instance.Value, valueTypes[0]), i.Value.UseGuidFormatter()));
            }

            if (valueType == typeof(DateTime))
            {
                return dicItems.Select(i => new KeyValuePair<object, object>(ConvertFactory.FromObject().ChangeType(i.Key.Instance.Value, valueTypes[0]), i.Value.UseDateTimeFormatter()));
            }

            if (valueType == typeof(TimeSpan))
            {
                return dicItems.Select(i => new KeyValuePair<object, object>(ConvertFactory.FromObject().ChangeType(i.Key.Instance.Value, valueTypes[0]), i.Value.UseTimeSpanFormatter()));
            }
