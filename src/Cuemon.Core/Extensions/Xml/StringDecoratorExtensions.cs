...
namespace Cuemon.Xml
{
    ...
    public static class StringDecoratorExtensions
    {
        ...
        {
            Validator.ThrowIfNull(decorator, nameof(decorator));
            if (Decorator.Enclose(decorator.Inner).StartsWith(StringComparison.OrdinalIgnoreCase, Decorator.Enclose(Alphanumeric.Numbers).ToEnumerable().Concat(new[] { "." } )))
            {
                var startIndex = 0;
                var numericsAndPunctual = new List<char>(Alphanumeric.Numbers.ToCharArray().Concat(new[] { '.' }));
                foreach (var c in decorator.Inner)
                {
                ...
                return SanitizeXmlElementName(Decorator.Enclose(decorator.Inner.Substring(startIndex)));
            }
        ...
        {
            Validator.ThrowIfNull(decorator, nameof(decorator));
            if (string.IsNullOrEmpty(decorator.Inner)) { return decorator.Inner; }
            var value = StringReplacePair.RemoveAll(decorator.Inner, '\x0001', '\x0002', '\x0003', '\x0004', '\x0005', '\x0006', '\x0007', '\x0008', '\x0011', '\x0012', '\x0014', '\x0015', '\x0016', '\x0017', '\x0018', '\x0019');
