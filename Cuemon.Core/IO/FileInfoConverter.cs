...
namespace Cuemon.IO
{
    ...
    public static class FileInfoConverter
    {
        ...
        public static T FromFile<T>(string fileName, Func<FileInfo, byte[], T> converter)
        {
            return FromFile(fileName, 0, converter);
        }
        ...
        /// <param name="converter">The function delegate that will handle the conversion.</param>
        /// <returns>An instance of <typeparamref name="T"/>.</returns>
        public static T FromFile<T>(string fileName, int bytesToRead, Func<FileInfo, byte[], T> converter)
        {
