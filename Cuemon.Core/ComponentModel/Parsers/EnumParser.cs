...
namespace Cuemon.ComponentModel.Parsers
{
    ...
    public class EnumParser : ITypeParser<EnumOptions>
    {
        ...
        /// <summary>
        /// Indicates whether the <paramref name="input"/> is valid by attempting to construct an enumeration of <typeparamref name="T"/>.
        /// </summary>
        /// <typeparam name="T">The type of the enumeration to validate.</typeparam>
        /// <param name="input">The <see cref="string"/> containing the name or value used to attempt to construct an <see cref="Enum"/>.</param>
        /// <param name="setup">The <see cref="EnumOptions"/> which may be configured.</param>
        /// <returns><c>true</c> if the <paramref name="input"/> is a valid enumeration; otherwise, <c>false</c>.</returns>
        public bool IsEnum<T>(string input, Action<EnumOptions> setup = null) where T : struct, IConvertible
        {
            if (string.IsNullOrWhiteSpace(input)) { return false; }
            return typeof(T).GetTypeInfo().IsEnum && TryParse<T>(input, out _, setup);
        }
    }
}